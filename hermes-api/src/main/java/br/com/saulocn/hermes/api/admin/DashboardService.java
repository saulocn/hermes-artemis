...
public class DashboardService {
    ...
    public AdminVOs.Stats stats() {
        // One pass over recipient instead of three counts. `processed` and `sent` are the only
        // state the schema carries, so the three buckets are derived from them here.
        ...
        String where = """
                where (cast(:filter as text) is null or lower(recipient_mail) like cast(:filter as text))
                  and (cast(:state as text) is null
                       or (cast(:state as text) = 'pending'   and not recipient_processed and not recipient_sent)
                       or (cast(:state as text) = 'inFlight'  and recipient_processed and not recipient_sent)
                       or (cast(:state as text) = 'delivered' and recipient_sent))
                """;
    ...
    public boolean retry(Long recipientId) {
        int updated = em.createQuery("update Recipient r set r.processed = false where r.id = :id")
